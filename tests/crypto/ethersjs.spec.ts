...
const mnemonic =
  "wine agree vacuum crowd describe damp chapter behind sweet tomato transfer earn";
...
describe("--ethereum--", () => {
  describe("Generate HD extended keys", () => {
    it("can generate xpub key", async () => {
      ...
      assert.equal(
        xpub,
        "xpub661MyMwAqRbcFqzMQsKoZo7Du8JrGrdNSTpMcLPjWzXLoqzMeiDos3dMundq4xjwKakHmbaTr5mG4QrDCg9vjDqWxMUVBzRbmU7RcGkzNcL"
      );
    });
    ...
    it("can generate xprv key", async () => {
      ...
      assert.equal(
        xprv,
        "xprv9s21ZrQH143K3MutJqnoCfAVM6UMsPuX5Etkowz7xezMw3fD7AuZKFJt4WGDyaxH9Y8htfaNtpRoo4zv373yTGkWoUaJQeybzRp4UGLcgn8"
      );
    });
  });
  ...
  describe("Generate wallets", () => {
    it("can generate random address", async () => {
      const { address, privateKey } = ethereum.createEthAddress({});
      assert.ok(address);
    });
